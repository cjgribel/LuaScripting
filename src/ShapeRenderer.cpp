...
struct unitcube_t
{
    ...
    const std::array<v3f, 8> vertices =
    {
        v3f {0.5f,-0.5f, 0.5f},
        v3f {0.5f, 0.5f, 0.5f},
        v3f {-0.5f, 0.5f, 0.5f},
    ...
    const std::array<vec3f, 6> tri_normals =
    {
        v3f {0.0f, 0.0f, 1.0f},
        v3f {0.0f, 0.0f, -1.0f},
        v3f {1.0f, 0.0f, 0.0f},
                 ...
                 std::vector<unsigned>& indices)
{
    ...
    for (int hi = 0; hi < hres; hi++)
    {
        ...
        {
            float theta = 2.0f*fPI * (float)ri/(rres-1);
            vec3f v = { d*cos(theta), d*sin(theta), z };
            vec3f n = { h/r*cos(theta), h/r*sin(theta), 1 };
...
void ImPrimitiveRenderer::init()
{
    ...
    push_states(BackfaceCull::True, DepthTest::True, Color4u::White, m4f_1);
...
void ImPrimitiveRenderer::push_quad(const v3f points[4],
                                    const v3f& n)
{
...
void ImPrimitiveRenderer::push_quad(const vec3f& pos,
                                    float scale)
{
    const auto [color, depth_test, cull_face] = get_states<Color4u, DepthTest, BackfaceCull>();
...
void ImPrimitiveRenderer::push_quad_wireframe()
{
...
void ImPrimitiveRenderer::push_cube()
{
...
void ImPrimitiveRenderer::push_cube_wireframe()
{
    const auto& transform = get_states<m4f>();
...
//void ImPrimitiveRenderer::push_circle_ring(float r,
//                                           const m4f& m,
//                                           float subdiv,
//                                           Color4u color,
...
void ImPrimitiveRenderer::push_line(const vec3f& pos0,
                                    const vec3f& pos1)
{
    ...
    line_vertices.push_back( {pos0, color} );
    line_vertices.push_back( {pos1, color} );
    
    line_hash[ldc].push_back( vertex_ofs+0 );
    line_hash[ldc].push_back( vertex_ofs+1 );
}

void ImPrimitiveRenderer::push_lines_from_cyclic_source(const LineVertex* vertices,
                                                        int start_index,
                                                        int nbr_vertices,
                                                        int max_vertices)
{
...
void ImPrimitiveRenderer::push_lines(const std::vector<v3f>& vertices,
...
void ImPrimitiveRenderer::push_lines(const v3f* vertices,
                                     size_t nbr_vertices,
...
void ImPrimitiveRenderer::push_grid(const vec3f& pos,
                                    unsigned size,
                                    unsigned resolution)
{
...
void ImPrimitiveRenderer::push_cone(const vec3f& from,
                                    const vec3f to,
                                    float r)
{
...
void ImPrimitiveRenderer::push_cone(float h,
                                    float r,
                                    bool flip_normals)
{
    const auto [color, depth_test, cull_face, M] = get_states<Color4u, DepthTest, BackfaceCull, m4f>();
...
void ImPrimitiveRenderer::push_cylinder(float h,
                                        float r,
                                        Ray* ray)
{
    const auto [color, depth_test, cull_face, M] = get_states<Color4u, DepthTest, BackfaceCull, m4f>();
...
void ImPrimitiveRenderer::push_sphere(float h, float r)
{
    const auto [color, depth_test, cull_face, M] = get_states<Color4u, DepthTest, BackfaceCull, m4f>();
    const auto vertex_ofs = polygon_vertices.size();
    const auto index_ofs = polygon_indices.size();
...
void ImPrimitiveRenderer::push_sphere_wireframe(float h, float r)
{
    const auto [color, depth_test, cull_face, M] = get_states<Color4u, DepthTest, BackfaceCull, m4f>();
    const auto vertex_ofs = polygon_vertices.size();
    const auto index_ofs = polygon_indices.size();
...
void ImPrimitiveRenderer::push_arrow(const vec3f& from,
                                     const vec3f& to,
...
void ImPrimitiveRenderer::push_helix(const vec3f& from,
                                     const vec3f& to,
                                     float r_outer,
                                     float r_inner,
                                     float revs)
...
void ImPrimitiveRenderer::push_helix(float length,
                                     float r_outer,
                                     float r_inner,
                                     float revs)
{
...
void ImPrimitiveRenderer::push_frustum(const mat4f& invProjView)
{
...
void ImPrimitiveRenderer::push_basis_basic(const m4f& basis, float arrlen)
{
...
void ImPrimitiveRenderer::push_basis_basic2d(const m4f& basis, float arrlen)
{
    const vec3f wpos = basis.column(3).xyz();
...
void ImPrimitiveRenderer::push_basis(const m4f& basis,
                                     float arrlen,
...
void ImPrimitiveRenderer::push_point(const vec3f& p, unsigned size)
{
    const auto [color, depth_test] = get_states<Color4u, DepthTest>();
...
void ImPrimitiveRenderer::push_points(const PointVertex* points,
                                      unsigned nbr_points,
                                      unsigned size)
{
    const auto& depth_test = get_states<DepthTest>();
...
//    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    ...
    if (polygon_hash.size())
    {
        ...
        // Uniforms
        glUniformMatrix4fv(glGetUniformLocation(lambert_shader, "PROJ_VIEW"), 1, 0, PROJ_VIEW.array);
        glUniform1f(glGetUniformLocation(lambert_shader, "ambient_ratio"), 0.6);
    ...
    if (line_hash.size())
    {
        ...
        glUniformMatrix4fv(glGetUniformLocation(line_shader, "PROJ_VIEW"), 1, 0, PROJ_VIEW.array);
...
//        glDisable(GL_DEPTH_TEST);
        ...
        glUniformMatrix4fv(glGetUniformLocation(point_shader, "PROJ_VIEW"), 1, 0, PROJ_VIEW.array);
